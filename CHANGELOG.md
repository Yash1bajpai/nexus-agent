...
- **Direct Module Invocation Entrypoint**: Added `src/__main__.py` to enable launching `nexus-agent` directly via `python -m nexus_agent_ai` or `python -m src` when `PATH` binary registration is skipped.
- **Windows Onboarding Troubleshooting**: Expanded `README.md` with explicit PowerShell execution policy (`Set-ExecutionPolicy`), pip file locking (`python -m pip install --upgrade pip`), and PATH isolation fixes.
